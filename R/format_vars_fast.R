  ...
  if(length(u_agg_levels) == 1){
    ...
    if(length(u_agg_levels) == 2){
      ...
      if(all(u_agg_levels == c(2, 3))){
        ...
        pixel_static_u <- unique(pixel_static)
        if(length(pixel_static) > length(pixel_static_u)){
          pixel_dynamic_u <- c()
          times_dynamic_u <- unique(times_dynamic)
          n_times <- length(times_dynamic_u)
          index_keep <- c()
          #reduce to unique observations
          pixel_unique <- unique(pixel_static)
          for(i in 1:n_static){
            obs_list[[which_static[i]]] <- obs_list[[which_static[i]]][match(pixel_static_u, pixel_static)]
          }

          for(t in 1:n_times){
            pixel_t <- pixel_dynamic[times_dynamic == times_dynamic_u[t]]
            index_keep <- c(index_keep,
                            which(times_dynamic == times_dynamic_u[t])[match(pixel_static_u, pixel_t)])
          }

          for(i in 1:n_dynamic){
            obs_list[[which_dynamic[i]]] <- obs_list[[which_dynamic[i]]][index_keep]
          }
          pixel_static <- pixel_static_u
          pixel_dynamic <- pixel_dynamic[index_keep]
        }
      ...
      if(all(u_agg_levels == c(1, 3))){
        ...
        for(i in 1:n_subset){
          ...
          if(length(which_times) == 1){
            time_use[i] <- which_times
          }else{
